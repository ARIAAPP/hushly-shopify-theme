/* Hushly — Cart Drawer Logic */
(function() {
  'use strict';

  var SHOP_DOMAIN = (window.hushlyTheme && window.hushlyTheme.shopDomain) || (window.Shopify && window.Shopify.shop) || window.location.hostname;
  ...
  function render() {
    ...
    document.dispatchEvent(new CustomEvent('hushly:cart-state', { detail: { isOpen: state.isOpen, items: state.items } }));
  }
    ...
    return '<div class="cart-line-item" style="display:flex; gap:0.75rem; border-bottom:1px solid var(--color-border); padding-bottom:1rem;">' +
      ...
      '<div style="flex:1; min-width:0;">' +
        '<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:0.5rem;">' +
          ...
          '<button type="button" data-remove="' + item.id + '" aria-label="Retirer" style="color:var(--color-muted-fg);">&times;</button>' +
        '</div>' +
        ...
        '<div style="margin-top:0.5rem; display:flex; align-items:center; justify-content:space-between;">' +
          '<div style="display:inline-flex; align-items:center; border-radius:9999px; border:1px solid var(--color-border);">' +
            '<button type="button" data-qty="dec" data-id="' + item.id + '" aria-label="Diminuer" style="padding:0.25rem 0.5rem; color:var(--color-muted-fg);">&minus;</button>' +
  ...
  document.addEventListener('click', function(e) {
    ...
    if (toggle) { e.preventDefault(); window.HushlyCart.toggle(); return; }
    var close = e.target.closest('[data-cart-close]');
    if (close) { e.preventDefault(); window.HushlyCart.close(); return; }
    var overlay = e.target.closest('[data-cart-overlay]');
    if (overlay) { window.HushlyCart.close(); return; }
    var remove = e.target.closest('[data-remove]');
    if (remove) { window.HushlyCart.removeItem(remove.dataset.remove); return; }
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && state.isOpen) window.HushlyCart.close();
  });
