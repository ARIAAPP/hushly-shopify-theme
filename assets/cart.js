/* Soufflo — Cart Drawer Logic */
(function() {
  'use strict';

  var SHOP_DOMAIN = (window.souffloTheme && window.souffloTheme.shopDomain) || (window.Shopify && window.Shopify.shop) || window.location.hostname;
  ...
  function render() {
    ...
    if (state.items.length === 0) {
      ...
      if (missing.length > 0 && $upsells && $upsellsList) {
        ...
        $upsellsList.innerHTML = missing.map(function(a) {
          return '<button type="button" class="cart-addon-btn" data-add-addon="' + a.id + '" style="width:100%; display:flex; align-items:center; justify-content:space-between; gap:0.75rem; border-radius:8px; border:1px solid var(--color-border); background:var(--color-bg); padding:0.5rem 0.75rem; text-align:left; cursor:pointer;">' +
    ...
    document.dispatchEvent(new CustomEvent('soufflo:cart-state', { detail: { isOpen: state.isOpen, items: state.items } }));
  }
    ...
    return '<div class="cart-line-item" style="display:flex; gap:0.75rem; border-bottom:1px solid var(--color-border); padding-bottom:1rem;">' +
        ...
        '<div style="flex:1; min-width:0;">' +
          '<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:0.5rem;">' +
            ...
            '<button type="button" data-remove="' + item.id + '" aria-label="Retirer" style="color:var(--color-muted-fg); font-size:1.25rem; line-height:1; cursor:pointer;">&times;</button>' +
          '</div>' +
          ...
          '<div style="margin-top:0.5rem; display:flex; align-items:center; justify-content:space-between;">' +
            '<div style="display:inline-flex; align-items:center; border-radius:9999px; border:1px solid var(--color-border);">' +
              '<button type="button" data-qty="dec" data-id="' + item.id + '" aria-label="Diminuer" style="padding:0.25rem 0.5rem; color:var(--color-muted-fg); cursor:pointer;">&minus;</button>' +
              '<span style="padding:0 0.5rem; font-size:0.875rem; font-weight:600;">' + item.quantity + '</span>' +
              '<button type="button" data-qty="inc" data-id="' + item.id + '" aria-label="Augmenter" style="padding:0.25rem 0.5rem; color:var(--color-muted-fg); cursor:pointer;">+</button>' +
            '</div>' +
            '<p style="font-size:0.875rem; font-weight:600; margin:0;">' + fmt(item.price * item.quantity) + '</p>' +
          '</div>' +
        '</div>' +
      '</div>';
  }
  ...
  document.addEventListener('click', function(e) {
    /* Open cart (header button) */
    var openBtn = e.target.closest('[data-cart-open]') || e.target.closest('[data-cart-toggle]');
    if (openBtn) { e.preventDefault(); window.SouffloCart.toggle(); return; }
    /* Close cart */
    var close = e.target.closest('[data-cart-close]');
    if (close) { e.preventDefault(); window.SouffloCart.close(); return; }
    /* Overlay click */
    var overlay = e.target.closest('[data-cart-overlay]');
    if (overlay) { window.SouffloCart.close(); return; }
    /* Remove item */
    var remove = e.target.closest('[data-remove]');
    if (remove) { window.SouffloCart.removeItem(remove.dataset.remove); return; }
    /* Quantity +/- */
    ...
    if (qtyBtn) {
      ...
      if (newQty <= 0) { window.SouffloCart.removeItem(id); } else { window.SouffloCart.setQuantity(id, newQty); }
      return;
    }
    /* Add addon */
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && state.isOpen) window.SouffloCart.close();
  });
